...
}```## 检查是否可以监听通知 - 接口说明：```txt- 检查此设备是否可以监听通知，如果可以监听，你可以使用websocket来监听android的实时通知
- 实时通知需要android9.0版本以上``` - 请求url：`http://[ip]:18080/api/checkNotification` - 请求方式：`GET` - 请求参数：    - 无 - 返回示例：```json{
	"code": 1,
	"data": "1"
...
}```## 获取所有联系人 - 请求url：`http://[ip]:18080/api/getAllContact` - 请求方式：`GET` - 请求参数：    - number：`13312341234` (不传或者传*的时候获取所有联系人，否则获取此号码对应的联系人) - 返回示例：```json{
	"code": 1,
	"data": [
		{
			"number": "13312341234",
...
data字段返回删除成功的数量
请谨慎调用此接口``` - 请求url：`http://[ip]:18080/api/deleteContact` - 请求方式：`GET` - 请求参数：    - number：`13312341234` (不传或者传*的时候删除所有联系人，否则删除此号码对应的联系人) - 返回示例：```json{
	"code": 1,
	"data": "0"
...
}```## 删除短信 - 接口说明：```txt由于android系统的限制，必须通过短信app删除短信，所以此接口不适用``` - 请求url：`http://[ip]:18080/api/deleteSms` - 请求方式：`GET` - 请求参数：    - number：`10010`  - 返回示例：    - 无## 模拟按键 - 接口说明：```txtvalue代表keyCode
...
<intent-filter>
   ...
   <category android:name="android.intent.category.LAUNCHER"/>
	...
	"code": 1,
	"data": "1"
}```## 清理通知栏功能 - 接口说明：```txt由于android的安全限制，高版本的android已经无法调用清理通知栏的功能``` - 请求url：`http://[ip]:18080/api/cancelAllNotifications` - 请求方式：`GET` - 请求参数：    - 无 - 返回示例：```json{
	"code": 1,
	"data": "1"
