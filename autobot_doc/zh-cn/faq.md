...
在 root 环境下，做好隐藏后，使用安全模式，能更好的避开某些程序的检测问题。

- 缺点是 ：
  - 无法获取布局信息，因此获取屏幕布局接口文字查找，选择器查找无法使用
  - 及 websocket 接口内的通知监听也将无法使用
- 如何开启安全模式
  - 默认的情况下，autobot 都是非安全模式，你可以调用开启安全模式的接口，来打开安全模式。
  - root 环境下，有安全模式的 magisk 包，开机启动便是安全模式。
